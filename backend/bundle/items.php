...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!isset($_POST['reseller_code'], $_POST['quantity'])) {
    echo json_encode(["status" => "error", "message" => "Missing required fields."]);
    exit();
  }

  $ol_code = $_POST['reseller_code'];
  $ol_qty = (int) $_POST['quantity'];
  $ol_status = 'On Order';
  $ol_seller = $users_code ?? '';
  $ol_reseller = $users_creator ?? '';
  $ol_state = $_SESSION['new_state'];
  $ol_country = $_SESSION['new_country'];
  ...
  $stmt->bind_param("ss", $rspoid, $ol_code);
  $stmt->execute();
  $result = $stmt->get_result();
  ...
  if ($existingOrder) {
    ...
    $stmt->bind_param("iiiiss", $new_qty, $new_points, $new_php, $new_subtotal, $rspoid, $ol_code);
    ...
    $stmt->bind_param("sssssisiddsds", $rspoid, $ol_desc, $now, $ol_seller, $ol_reseller, $ol_points, $ol_code, $ol_qty, $ol_php, $ol_price, $ol_country, $ol_subtotal, $ol_status);
